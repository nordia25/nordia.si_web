  ...
  useEffect(() => {
    ...
    try {
      ctx = gsap.context(() => {
        ...
        // Get the length of the text stroke for animation (with fallback)
        let textLength = 300; // Fallback value
        try {
          const computedLength = textElement.getComputedTextLength();
          if (computedLength > 0) {
            textLength = computedLength;
          }
        } catch {
          console.warn('getComputedTextLength failed, using fallback');
        }
